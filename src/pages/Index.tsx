...
const Index = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (USE_MOCK_DATA) {
        ...
        // Helper: convert Blob to data URL (base64) so it can be saved to localStorage
        ...
        if (contentType.includes("application/json") || contentType.includes("text/")) {
          ...
          // Extract image from JSON response (adjust based on your webhook's response format)
          if (data && typeof data === "object") {
            if (data.image) {
              ...
              console.error("Unknown JSON response format:", data);
              throw new Error("No image found in JSON response. Check console for response structure.");
            }
          } else if (typeof data === "string") {
            imageUrl = data;
          }
        } else {
          // Assume binary image/blob (e.g. image/webp, image/png)
          const blob = await response.blob();
          // Convert to data URL so it can be displayed and persisted
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (cakeImage.startsWith("data:")) {
        // Data URL: can be used directly
        link.href = cakeImage;
      } else {
        // Remote URL: fetch and create object URL
        const response = await fetch(cakeImage);
        const blob = await response.blob();
        const url = window.URL.createObjectURL(blob);
        link.href = url;
        // revoke after a short timeout to ensure download starts
